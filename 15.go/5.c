/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2017年11月17日 星期五 17时58分43秒
 ************************************************************************/
...
RBTNode *erase_maintain(RBTNode *node) {
    ...
    if (node->lchild->black == 2 && node->rchild->black == 0) {
        ...
        if (rchild->lchild->black != 0 && rchild->rchild->black != 0) {
            lchild->black = 1;
            rchild->black = 0;
            node->black += 1;
            return node;
        }
        if (rchild->rchild->black != 0 && rchild->lchild->black == 0) {
            node->rchild = right_rotate(node->rchild);
            node->rchild->black = 1;
        ...
        if (lchild->rchild->black != 0 && lchild->lchild->black != 0) {
            rchild->black = 1;
            lchild->black = 0;
            node->black += 1;
            return node;
        }
        if (lchild->lchild->black != 0 && lchild->rchild->black == 0) {
            node->lchild = left_rotate(node->lchild);
            node->lchild->black = 1;
