/*************************************************************************
	> File Name: red_black_tree_test_4.c
	> Author: 
	> Mail: 
	> Created Time: 2017年11月17日 星期五 17时15分39秒
 ************************************************************************/
...
int has_child(RBTNode *node) {
    return !node->lchild->black || !node->rchild->black;
}

RBTNode *insert_maintain(RBTNode *node) {
    if (node->black == 0) return node;
    if (!has_child(node)) return node;
    RBTNode *rchild = node->rchild, *lchild = node->lchild;
    if (!has_child(lchild) && !has_child(rchild)) return node;
    if (lchild->black == 0 && rchild->black == 0) {
        lchild->black = rchild->black = 1;
        node->black = 0;
        return node;
    } else if (lchild->black == 0 && has_child(lchild)) {
        if (lchild->lchild->black != 0) {
            node->lchild = left_rotate(node->lchild);
        }
        node = right_rotate(node);
        node->lchild->black = 1;
    } else if (rchild->black == 0 && has_child(rchild)) {
...
RBTNode *predessor(RBTNode *node) {
    RBTNode *temp = node->lchild;
...
RBTNode *erase_maintain(RBTNode *node) {
    ...
    if (lchild->black == 2 && rchild->black == 0) {
        ...
        if(rchild->lchild->black == 1 && rchild->rchild->black == 1) {
            lchild->black = 1;
            rchild->black = 0;
            node->black++;
            return node;
        ...
        if(lchild->rchild->black == 1 && lchild->lchild->black == 1) {
            rchild->black = 1;
            lchild->black = 0;
            node->black++;
            return node;
